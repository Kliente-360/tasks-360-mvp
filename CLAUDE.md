...
- A versão é declarada em **dois lugares** durante a coexistência Alpine + Next:
  - `lib/helpers.js` (Alpine, app em produção até cutover)
  - `web/src/components/app-nav.tsx` constante `APP_VERSION` (Next, atual em preview Vercel)
  - **Bumpar os dois juntos.** Pós-cutover sobra só o do Next.
