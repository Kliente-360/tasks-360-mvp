...
function app() {
  ...
  const base = {
    ...
    _dataRev: 0,
    f: { q: '', cliente: '', projeto: '', pessoa: '', pri: '', complexidade: '', status: 'abertas', tag: '', origem: '' /* '' | 'ia' | 'humano' */ },
    newTag: '',
    sortKeys: [],
    sortPanelOpen: false,
    sortOptions: [
    ...
    quickAddTitle: '',
    selectedIds: [],
    bulkPending: { subetapa: '', pessoa: '', cliente: '', projeto: '', prioridade: '', prazo: '', esforco: '' },
    triagemFilter: { semResp: false, semPrazo: false, semEsforco: false, origem: '' /* '' | 'ia' | 'humano' */ },
    ...
    tabsList: [
      ...
      { key: 'triagem', label: 'Triagem',  roles: ['admin'] },
    ...
    // ===================== INIT / PERSISTÊNCIA =====================
    async init() {
      // Idempotência: Alpine 3 auto-chama `init()` se o data factory expõe esse
      // método. O `<body x-data="app()" x-init="init()">` aciona DUAS vezes
      // (auto + explícito), o que dobrava `addEventListener('keydown')` e fazia
      // o Cmd+K toggle do palette disparar 2x num único press (false→true→false,
      // palette piscava). Guard garante uma init só, mesmo com x-init redundante.
      ...
      this.$watch('currentPessoa', (p) => {
        ...
        if (!hasTabParam && p.role === ROLE.ADMIN) this.tab = 'brief';
        // Shortcut PWA ?tab=brief pra não-admin cai no Dashboard
        if (hasTabParam && this.tab === 'brief' && p.role !== ROLE.ADMIN) this.tab = 'dash';
      });
      ...
      this.$watch('editing', (next) => {
        ...
        if (this.clienteWebhookEnabled(next.clienteId)) return;  // webhook client: save manual only
      ...
      // Error handler global: erro não-tratado (em getter, handler, promise)
      // pode quebrar uma aba em silêncio. Toast dá visibilidade mínima sem
      // poluir — throttle de 1 por 8s pra não floodar em erro recorrente.
      const _onGlobalError = (msg) => {
        const now = Date.now();
        if (this._lastErrToastAt && now - this._lastErrToastAt < 8000) return;
        this._lastErrToastAt = now;
        try { this.toast('error', 'Algo deu errado: ' + String(msg || 'erro inesperado'), 6000); } catch (_) {}
      };
      window.addEventListener('error', (e) => {
        // Ignora falha de carregamento de recurso (script/img/link/css):
        // o evento 'error' borbulha desses elementos, mas e.target é o
        // elemento — não é erro de runtime do app.
        if (e.target && e.target !== window && e.target.tagName) return;
        // Ignora "Script error." de script cross-origin (CDNs): o browser
        // esconde o detalhe por segurança e o `e.error` vem vazio — sem
        // nada acionável. Só toastamos erro com Error object de verdade,
        // que só existe pra script same-origin (nosso lib/*.js).
        if (!e.error) return;
        console.error('[global error]', e.error);
        _onGlobalError(e.message);
      });
      window.addEventListener('unhandledrejection', (e) => {
        console.error('[unhandled rejection]', e.reason);
        _onGlobalError(e.reason && (e.reason.message || e.reason));
      });
    ...
    async ensureCurrentPessoa(session) {
      ...
      const r1 = await sb.from('pessoas').select('id, nome, email, user_id, invited_at, role, cliente_id, cliente_principal_id, cliente_secundario_id, capacidade_horas_semana,skills,senioridade,is_ceo').eq('user_id', userId).maybeSingle();
      if (r1.error) {
      ...
      if (email) {
        const r2 = await sb.from('pessoas').select('id, nome, email, user_id, invited_at, role, cliente_id, cliente_principal_id, cliente_secundario_id, capacidade_horas_semana,skills,senioridade,is_ceo').ilike('email', email).maybeSingle();
        if (r2.error) {
    ...
    _hydrateTaskFromUrl() {
      ...
      const p = new URLSearchParams(window.location.search);
      // ?new=1 — PWA shortcut "Nova task" (long-press ícone)
      if (p.has('new')) {
        this.$nextTick(() => this.openNew());
        return;
      }
