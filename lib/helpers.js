...
(function () {
  ...
  //   MAJOR: era arquitetural (1 = modular sem build; 2 = Next/Drizzle).
  //   MINOR: bloco de releases dentro da era. Bumpa em mudanças grandes de
  //          UX/dados (nova aba, novo modelo). 02 abriu pós ciclo de design.
  //   BUILD: sequencial, +1 a cada commit em main. Zera no bump de MINOR.
  //          Independente do número do PR no GitHub.
  const APP_VERSION = 'v1.02.095';
  ...
  // Escapa string pra interpolação segura em HTML (uso com x-html).
  // Getters que montam HTML de narrativa (memo executivo) interpolam
  // dados do usuário — título de task, nome de cliente/pessoa — que
  // podem conter `<`, `>`, etc. Sem escapar = XSS armazenado.
  ...
  // Ocupação restante = esforço ainda por trabalhar (floor 0).
  // Desconta horas já registradas do esforço estimado.
  // Usado pra capacidade prospectiva (tasks abertas).
  function effOcupacao(t) {
    return Math.max(0, effEsforco(t) - (Number(t && t.tempoRealHoras) || 0));
  }
  ...
  // Soma de ocupação restante num bucket de tasks.
  function sumEffOcupacao(tasks) {
  ...
  Object.assign(window, {
    ...
    normalizeTag, suggest, fmtDate, fmtDateShort, atrasada, escapeHtml,
    cargaNivelFromPctCap, effEsforco, effOcupacao, effTamanho,
    triageFailures, needsTriage,
    weekStartMonday, addDays, effPrazoForAnalysis,
    taskWeekIndex, bucketTasksByWeek, sumEffEsforco, sumEffOcupacao,
