...
const TASK_FIELDS = [
  ...
  // Origem: true quando a task foi criada por automação de IA (Cowork etc).
  // Setado via edge function ingest-task; UI mostra chip 🤖 IA.
  ['criadoPorIa',     'criado_por_ia',     F.boolFalse],
  // Task privada: visível apenas ao dono (pessoa_id). Só CEO pode criar.
  ['privada',         'privada',           F.boolFalse],
];
...
const CLIENTE_FIELDS = [
  ...
  ['arquivadoEm',     'arquivado_em',    { from: r => r || null, default: null }],
  // Domínios de email do cliente (ex: ["bodytech.com.br"]). Sempre array,
  // nunca null. Normalização (lowercase, strip @, trim) feita ao salvar.
  ['dominios',        'dominios',        F.arr],
  // webhook_enabled: autosave desativado no modal de task pra este cliente.
  // Tasks desse cliente só salvam no clique explícito de "Salvar".
  ['webhookEnabled',  'webhook_enabled', F.boolFalse],
];
