...
(function () {
  ...
  function makeCoreDataView() {
    return {
      ...
      // Captura mudanças em quantidade + status_em/subetapa_em + _dataRev (bumpado
      // explicitamente em saveTask). Isso garante que mudanças em campos sem
      // timestamp (prazo, pessoaId, esforço, prioridade, complexidade) também
      // invalidem memos como `triagemTasks`. Sem o _dataRev, triagem cacheada
      // ficava stale ao preencher prazo no modal.
      get _tasksSig() {
        let s = this.tasks.length + (this._dataRev | 0);
