  ...
  function makeCoreDataView() {
    return {
      ...
      get reportClientesExec() {
        ...
        for (const h of baseHealth) {
          ...
          for (const t of cliTasks) {
            ...
            else if (t.status !== STATUS.CONCLUIDO) horas += this.effOcupacao(t);
          }
      ...
      async exportPDF() {
        ...
        // Chart.js é carregado sob demanda (lazy) — garante antes de montar.
        try { await this._ensureChartJs(); } catch (_) { /* PDF sai sem charts */ }
      ...
      clienteWebhookEnabled(id) { const c = this.clientesById.get(id); return !!(c && c.webhookEnabled); },
      ...
      effOcupacao(t) { return effOcupacao(t); },
      ...
      get stats() {
        ...
        const sum = arr => arr.reduce((a,b) => a + this.effOcupacao(b), 0);
        return {
