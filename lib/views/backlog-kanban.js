  ...
  function makeBacklogKanbanView() {
    return {
      ...
      _computeFiltered() {
        ...
        // Chain vazio: sem sort explícito, mantém ordem de criação desc
      ...
      sortBy(key) {
        ...
        if (idx === -1) {
          // Não está no chain: 1º clique → entra como primária asc
          const rest = this.sortKeys.filter(s => s.key !== key).slice(0, 2);
          this.sortKeys = [{ key, dir: 'asc' }, ...rest];
        } else if (idx > 0) {
          // Está no chain como tiebreaker: promove a primária asc (reinicia ciclo)
          const rest = this.sortKeys.filter(s => s.key !== key).slice(0, 2);
          this.sortKeys = [{ key, dir: 'asc' }, ...rest];
        } else if (this.sortKeys[0].dir === 'asc') {
          // É primária asc: 2º clique → vira desc
          this.sortKeys = [{ key, dir: 'desc' }, ...this.sortKeys.slice(1)];
        } else {
          // É primária desc: 3º clique → remove do chain
          this.sortKeys = this.sortKeys.slice(1);
        }
      },
      sortIcon(key) {
        const idx = this.sortKeys.findIndex(s => s.key === key);
      ...
      kanbanCol(status) {
        const arr = this._visibleTasks.filter(t =>
          ...
          && (!this.f.projeto || t.projetoId === this.f.projeto)
