  ...
  function makeBacklogKanbanView() {
    return {
      ...
      get _backlogFilterSig() {
        const f = this.f;
        return this._tasksSig + '|' + this.showArchivedTasks + '|' + this.sortKey + '|' + this.sortDir +
      ...
      clearSelection() { this.selectedIds = []; this.bulkPending = { subetapa: '', pessoa: '', prioridade: '', prazo: '', esforco: '' }; },
      async bulkSave() {
        const p = this.bulkPending;
        const hasAny = p.subetapa || p.pessoa || p.prioridade || p.prazo || p.esforco !== '';
        ...
        this.bulkPending = { subetapa: '', pessoa: '', prioridade: '', prazo: '', esforco: '' };
      },
