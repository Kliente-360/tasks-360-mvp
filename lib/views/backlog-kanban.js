...
(function () {
  ...
  function makeBacklogKanbanView() {
    return {
      ...
      // Sticky thead via JS · CSS position:sticky não funcionou (mesmo com
      // translateZ trick, -webkit-sticky, top:64px, todas as variações). Vamos
      // de overlay clonado: cria um <div> fixed na primeira vez que tab='backlog'
      // ativa, clona o <thead> dentro de uma mini-table com mesmo colgroup pra
      // preservar widths, e mostra/esconde via scroll listener.
      setupStickyThead() {
        if (this._stickyTheadInit) return;
        this._stickyTheadInit = true;
        this.$nextTick(() => {
          const wrap = document.querySelector('.tbl-wrap');
          if (!wrap) return;
          const origTable = wrap.querySelector('.tbl');
          if (!origTable) return;
          const origThead = origTable.querySelector('thead');
          const origColgroup = origTable.querySelector('colgroup');
          if (!origThead) return;

          // Container floating no body — fica fora da hierarquia da tabela,
          // sem stacking-context bloqueando.
          const floating = document.createElement('div');
          floating.className = 'tbl-thead-floating';
          floating.style.cssText = 'position:fixed;top:64px;left:0;z-index:30;display:none;pointer-events:none;';
          document.body.appendChild(floating);

          // Rebuild HTML do clone (thead + colgroup numa mini-table com
          // table-layout:fixed e width fixo do wrap pra alinhamento perfeito).
          const rebuildClone = () => {
            const cg = origColgroup ? origColgroup.outerHTML : '';
            const th = origThead.outerHTML;
            floating.innerHTML = '<table class="tbl" style="table-layout:fixed;width:' + wrap.offsetWidth + 'px;">' + cg + th + '</table>';
          };

          const update = () => {
            const r = wrap.getBoundingClientRect();
            const APP_HEADER = 64;
            const visibleBelow = r.bottom > APP_HEADER + 80; // ainda há tabela visível
            const wrapAboveHeader = r.top < APP_HEADER;
            if (wrapAboveHeader && visibleBelow) {
              if (floating.style.display === 'none') rebuildClone();
          ...
          window.addEventListener('resize', () => { rebuildClone(); update(); }, { passive: true });
          // Quando filtros/sort/grupo mudam, dom dos cells/widths podem mudar — rebuild.
          this.$watch('filtered', () => { rebuildClone(); update(); });
          update();
        });
      },
