  ...
  function makeTaskModalView() {
    return {
      ...
      openEdit(t) {
        ...
        // Ticker reativo pro autosaveLabel atualizar "salvo · agora" → "salvo · há 30s" sem input.
        if (this._autosaveTimer) clearInterval(this._autosaveTimer);
        this._autosaveTimer = setInterval(() => { this.autosaveTick++; }, 5000);
        this.editingCommentId = '';
        this.editingCommentDraft = '';
        this.newDepId = '';
        this.editingComments = [];
      ...
      // String "salvo · há Ns" para o indicador.
      // Reativa via lastSavedAt + _autosaveTick (incrementado a cada 5s enquanto modal aberto).
      autosaveLabel() {
        // Lê autosaveTick e usa o valor numa comparação trivial — garante que Alpine
        // registre essa property como dependência do x-text. Sem isso o getter não
        // re-roda quando o ticker incrementa.
        const _tick = this.autosaveTick || 0;
        ...
        if (this.saveState === 'saved' && this.lastSavedAt) {
          ...
          if (_tick < 0) return ''; // dead branch, só pra usar _tick e travar dependência
