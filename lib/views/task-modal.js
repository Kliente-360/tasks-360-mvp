  ...
  function makeTaskModalView() {
    return {
      ...
      openNew() {
        ...
        this.newReply = '';
        this.newDepId = '';
        // Mobile: abre na aba "detalhes" pra ver o form imediatamente
        // (conversa/histórico não fazem sentido em task ainda não salva).
        // Desktop: usa o layout 2 colunas, modalTab só afeta o pane direito.
      ...
      async saveTask(opts) {
        ...
        if (e.id) {
          ...
          if (i >= 0) {
            ...
            // Whole-array reassign: defesa contra edge case onde Alpine não
            // propaga mutação `tasks[i] = newObj` pra computeds dependentes.
            // Bug observado: tabela do Backlog mostrando status antigo após
            // salvar via modal. Mesma fix usada em refreshTasks/loadFull.
            this.tasks = this.tasks.map((t, idx) => idx === i ? optimistic : t);
            this._touchTasks();
          }
          // Aguarda update no banco ANTES de fechar modal — evita race onde
          // user vê modal fechado mas update falhou + toast aparece no ar.
          // `.select().maybeSingle()` confirma persistência: se RLS bloqueou
          // o update silenciosamente, `data` vem null e detectamos.
          const { data: updated, error } = await sb.from('tasks')
            .update(payload).eq('id', e.id)
            .select(this._TASK_LIGHT_COLS).maybeSingle();
          if (error) {
            if (prev) { this.tasks = this.tasks.map((t, idx) => idx === i ? prev : t); this._touchTasks(); }
            if (!silent) this.toast('error', 'Erro ao salvar: ' + error.message);
            else throw new Error(error.message);
            return;
          }
          if (!updated) {
            // RLS denegou ou row não existe — reverter e avisar
            if (prev) { this.tasks = this.tasks.map((t, idx) => idx === i ? prev : t); this._touchTasks(); }
            const msg = 'Sem permissão pra salvar ou tarefa foi removida.';
            if (!silent) this.toast('error', msg);
            else throw new Error(msg);
            return;
          }
          // Sucesso: sincroniza local com a versão canônica do banco. O trigger
          // `sync_task_status_from_subetapa` deriva `status` de `subetapa`, então
          // a fonte da verdade é o que voltou do DB. `descricao` não vem nas
          // light cols — preserva o valor local (lazy load semantic).
          if (i >= 0) {
            const fromDb = taskFromDb(updated);
            const preservedDescricao = (e.descricao !== undefined) ? (e.descricao || '') : prev.descricao;
            fromDb.descricao = preservedDescricao;
            this.tasks = this.tasks.map((t, idx) => idx === i ? fromDb : t);
          }
          // Invalida memos (triagem, heurísticas, etc.) dependentes de tasks.
          this._touchTasks();
          // Fecha modal só agora (botão Salvar) ou mantém aberto (autosave).
      ...
      deleteTask() {
        this.askConfirm('Excluir esta tarefa? Esta ação não pode ser desfeita.', async () => {
          ...
          const prev = i >= 0 ? this.tasks[i] : null;
          if (i >= 0) { this.tasks.splice(i, 1); this._touchTasks(); }
          this.modal = false;
          await this._purgeTaskStorage(id);
          const { error } = await sb.from('tasks').delete().eq('id', id);
          if (error) {
            if (prev) { this.tasks.splice(i, 0, prev); this._touchTasks(); }
