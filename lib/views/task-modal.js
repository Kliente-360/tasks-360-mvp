  ...
  function makeTaskModalView() {
    return {
      ...
      openNew() {
        ...
        this.modal = true;
        this.$nextTick(() => {
          const left = document.querySelector('.tmodal-left');
          if (left) left.scrollTop = 0;
          if (this.$refs.tituloInput) this.$refs.tituloInput.focus();
        });
      },
      openEdit(t) {
      ...
      async loadDescricao(taskId) {
        ...
        if (error) return;
        const left = document.querySelector('.tmodal-left');
        const prevScroll = left ? left.scrollTop : 0;
        ...
        // Restaura scroll após Alpine rerenderizar o textarea (evita scroll pra descrição).
        this.$nextTick(() => { if (left) left.scrollTop = prevScroll; });
      },
      async postComment() {
      ...
      async saveTask(opts) {
        ...
        if (e.id) {
          ...
          const prev = this.tasksById.get(e.id) || null;
          ...
          if (prev) {
            ...
            this._replaceTask(e.id, optimistic);
          }
          ...
          if (error) {
            if (prev) this._replaceTask(e.id, prev);
          ...
          if (prev) {
            ...
            fromDb.descricao = (e.descricao !== undefined) ? (e.descricao || '') : prev.descricao;
            this._replaceTask(e.id, fromDb);
          }
          ...
          if (!silent) this.modal = false;
          if (data) this._upsertTask(taskFromDb(data));
      ...
      deleteTask() {
        this.askConfirm('Excluir esta tarefa? Esta ação não pode ser desfeita.', async () => {
          ...
          const prev = this._removeTask(id);
          this.modal = false;
          await this._purgeTaskStorage(id);
          const { error } = await sb.from('tasks').delete().eq('id', id);
          if (error) {
            if (prev) this._upsertTask(prev);
