...
(function () {
  ...
  function makeAnexosView() {
    return {
      ...
      _TASK_LIGHT_COLS: 'id,titulo,cliente_id,projeto_id,pessoa_id,prioridade,esforco,complexidade,prazo,status,subetapa,bloqueado_por,visivel_cliente,criado_em,status_em,subetapa_em,ordem,tags,checklist,reopen_count,tipo_trabalho,tempo_real_horas,external_source,external_id,arquivado_em,criado_por_ia',
      ...
      async _loadFull() {
        try {
          const [c, p, ps, t, h, td] = await Promise.all([
            sb.from('clientes').select('id,nome,tier,eh_interno,arquivado_em').order('nome'),
      ...
      async _loadPortal() {
        ...
        try {
          const [c, p, ps, t] = await Promise.all([
            sb.from('clientes').select('id,nome,tier,eh_interno,arquivado_em').eq('id', cid),
      ...
      async refreshClientes() {
        const { data, error } = await sb.from('clientes').select('id,nome,tier,eh_interno,arquivado_em').order('nome');
