    ...
    return {
      ...
      subscribeRealtime() {
        ...
        sb.channel('kliente360-changes')
          .on('postgres_changes', { event: '*', schema: 'public', table: 'tasks'    }, (payload) => this._applyTaskRealtime(payload))
          .on('postgres_changes', { event: '*', schema: 'public', table: 'clientes' }, () => this._scheduleRefetch('clientes'))
          .on('postgres_changes', { event: '*', schema: 'public', table: 'projetos' }, () => this._scheduleRefetch('projetos'))
      ...
      _applyTaskRealtime(payload) {
        try {
          ...
          const incoming = taskFromDb(row);
          const i = this.tasks.findIndex(t => t.id === row.id);
          if (i >= 0) {
            this.tasks = this.tasks.map((t, idx) => idx === i ? incoming : t);
          } else {
            this.tasks = [incoming, ...this.tasks];
          }
          this._touchTasks();
        } catch (e) {
