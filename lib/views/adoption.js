...
(function () {
  ...
  function makeAdoptionView() {
    return {
      ...
      // 5 alertas acionáveis. Cada um drive uma ação clara — agendar 1:1,
      // rodar onboarding, investigar feature, decidir promover/descontinuar,
      // investigar regressão macro. Sem informativos "positivos" (já mostrado
      // nos indicadores) e sem soft signals (engajamento parcial, picos).
      get usageAlertas() {
        const out = [];
        ...
        // A1 · Pessoa silenciosa há +7d → agendar 1:1
        for (const p of pess) {
          if (p.kind === 'silenciosa') {
            out.push({
              severity: 'alta', kind: 'silenciosa',
              titulo: `${p.nome} silenciosa há ${p.ultimo_dias || '?'}d`,
              detalhe: 'tinha histórico ativo · agendar 1:1',
            });
          }
        }

        // A2 · Pessoa nunca usou → rodar onboarding
        for (const p of pess) {
          if (p.kind === 'nunca') {
            out.push({
              severity: 'alta', kind: 'nunca_logou',
              titulo: `${p.nome} nunca usou`,
              detalhe: 'convidado há +14d · rodar onboarding',
            });
          }
        }

        // A3 · Feature core em colapso → investigar regressão
        for (const f of feats) {
          if (f.kind === 'em-queda' && f.v7Prev >= 10) {
            out.push({
              severity: 'alta', kind: 'feature_colapso',
              titulo: `${f.event} em colapso`,
              detalhe: `${f.v7Prev} → ${f.v7} eventos (${f.delta}%) · investigar regressão`,
            });
          }
        }

        // A4 · Feature órfã (<10% adoção) → decidir promover ou descontinuar
        for (const f of feats) {
          if (f.kind === 'orfa') {
            out.push({
              severity: 'media', kind: 'feature_orfa',
              titulo: `${f.event} órfã`,
              detalhe: `${f.pct}% adoção · decidir promover ou descontinuar`,
            });
          }
        }

        // A5 · Adoption macro em queda forte → investigar causa
        if (weeks.eventos.prev >= 50 && weeks.eventos.pct <= -30) {
          out.push({
            severity: 'alta', kind: 'macro_queda',
            titulo: 'Adoption geral caindo',
            detalhe: `${weeks.eventos.prev} → ${weeks.eventos.atual} eventos (${weeks.eventos.pct}%) · investigar causa`,
          });
        }

        // Ordena por severidade. Cap 5 — disciplina de "ações da semana".
      ...
      // Indicadores de sucesso · adoção interna (P0 do diagnóstico v2).
      // 4 métricas com sinal verde/amarelo/vermelho contra metas declaradas,
      // mais conclusão narrativa heurística. Sem IA — thresholds calibrados
      // no diagnóstico estratégico §6 (metas de 30d → 60d).
      get adoptionSuccessIndicators() {
        // Excluir cliente interno (Kliente 360 bucket) das contagens — análise é sobre uso real.
        const internosIds = new Set(this.clientes.filter(c => c.ehInterno).map(c => c.id));
        const pessoasInternas = this.pessoas.filter(p => p.role !== ROLE.CLIENTE);
        const ids = new Set(pessoasInternas.map(p => p.id));

        // ---- 1. DAU/WAU (engajamento contínuo) ----
        const dw = this.usageDauWau;
        const dauWauRatio = dw.wau30 > 0 ? Math.round((dw.dau7 / dw.wau30) * 100) : 0;
        const dauWauSig = dauWauRatio >= 70 ? 'verde' : (dauWauRatio >= 50 ? 'amarelo' : 'vermelho');

        // ---- 2. Sessões/dia média (densidade de uso) ----
        const cut7 = Date.now() - 7 * 86400000;
        const sessionByDay = new Map(); // dia → Set(session_id)
        for (const e of this.usageEvents) {
          const ts = new Date(e.ts).getTime();
          if (ts < cut7) continue;
          if (!ids.has(e.pessoa_id)) continue;
          if (!e.session_id) continue;
          const dia = e.ts.slice(0, 10);
          if (!sessionByDay.has(dia)) sessionByDay.set(dia, new Set());
          sessionByDay.get(dia).add(e.session_id);
        }
        const totalSessions = [...sessionByDay.values()].reduce((s, set) => s + set.size, 0);
        const sessoesPorDia = sessionByDay.size > 0 ? Math.round((totalSessions / 7) * 10) / 10 : 0;
        const sessoesSig = sessoesPorDia >= 5 ? 'verde' : (sessoesPorDia >= 3 ? 'amarelo' : 'vermelho');

        // ---- 3. Comments públicos / semana (engajamento de conversa) ----
        const commentsPub7d = (this.mvpComments || []).filter(c => {
          if (!c.visivel_cliente) return false;
          const ts = new Date(c.criado_em || c.posted_em).getTime();
          return ts >= cut7;
        }).length;
        const commentsSig = commentsPub7d >= 20 ? 'verde' : (commentsPub7d >= 10 ? 'amarelo' : 'vermelho');

        // ---- 4. % tasks bem triadas (qualidade do cadastro) ----
        // Tasks ativas (não concluídas, não arquivadas) excluindo cliente interno.
        // "Bem triada" = needsTriage(t) === false (helper puro).
        const tasksAtivas = this.tasks.filter(t =>
          !t.arquivadoEm && t.status !== STATUS.CONCLUIDO && !internosIds.has(t.clienteId)
        );
        const tasksTriadas = tasksAtivas.filter(t => !needsTriage(t)).length;
        const pctTriadas = tasksAtivas.length > 0 ? Math.round((tasksTriadas / tasksAtivas.length) * 100) : 100;
        const triadasSig = pctTriadas >= 80 ? 'verde' : (pctTriadas >= 60 ? 'amarelo' : 'vermelho');

        // ---- 5. Retention semanal (continuidade de uso) ----
        // % pessoas ativas esta sem (0-6d) que TAMBÉM estavam ativas sem ant (7-13d).
        // Métrica clássica SaaS de retention W1: indica continuidade vs trial users.
        const cut14 = Date.now() - 14 * 86400000;
        const ativasEstaSem = new Set();
        const ativasSemAnt = new Set();
        for (const e of this.usageEvents) {
          if (!e.pessoa_id || !ids.has(e.pessoa_id)) continue;
          const ts = new Date(e.ts).getTime();
          if (ts >= cut7) ativasEstaSem.add(e.pessoa_id);
          else if (ts >= cut14) ativasSemAnt.add(e.pessoa_id);
        }
        let retidasCount = 0;
        for (const pid of ativasEstaSem) if (ativasSemAnt.has(pid)) retidasCount++;
        const retention = ativasSemAnt.size > 0 ? Math.round((retidasCount / ativasSemAnt.size) * 100) : 0;
        const retentionSig = retention >= 80 ? 'verde' : (retention >= 60 ? 'amarelo' : 'vermelho');

        // ---- 6. Movimentação 7d (% tasks ativas que mudaram de subetapa) ----
        // "Calor" do pipeline: tasks ativas que receberam ao menos 1 update
        // de subetapa/status nos últimos 7 dias. Usa subetapaEm/statusEm
        // (timestamp da última mudança) — subestima frequência mas captura
        // "essa task moveu na semana?" binário, que é o que importa.
        // Compara com semana anterior (7-13d) pra delta.
        let movEsta = 0, movAnt = 0;
        for (const t of tasksAtivas) {
          const ts = Math.max(t.subetapaEm || 0, t.statusEm || 0);
          if (ts >= cut7) movEsta++;
          else if (ts >= cut14) movAnt++;
        }
        const totalAtivas = tasksAtivas.length;
        const pctMov7d = totalAtivas > 0 ? Math.round((movEsta / totalAtivas) * 100) : 0;
        const pctMovAnt = totalAtivas > 0 ? Math.round((movAnt / totalAtivas) * 100) : 0;
        const deltaMov = pctMov7d - pctMovAnt;
        const movSig = pctMov7d >= 60 ? 'verde' : (pctMov7d >= 40 ? 'amarelo' : 'vermelho');
        const movDeltaTxt = deltaMov > 0 ? `↑ +${deltaMov}pp vs sem ant` :
                            deltaMov < 0 ? `↓ ${deltaMov}pp vs sem ant` :
                            '= vs sem ant';
        ...
        const cards = [
          {
            ...
            value: dauWauRatio + '%',
            sig: dauWauSig,
            metaText: 'meta 30d ≥ 70%',
            detalhe: `${dw.dau7} ativos hoje / ${dw.wau30} em 30d`,
            help: 'Daily Active / Weekly Active. Indica retenção: % das pessoas que entraram no mês voltam na última semana.',
          },
          {
            key: 'sessoes',
            label: 'Sessões/dia',
            value: sessoesPorDia.toFixed(1).replace('.', ','),
            sig: sessoesSig,
            metaText: 'meta 30d ≥ 5',
            detalhe: `${totalSessions} sessões em 7d`,
            help: 'Média de sessões únicas por dia (últimos 7d). Mede densidade de uso — quem usa diário, abre 5+ sessões.',
          },
          {
            key: 'comments',
            label: 'Comments/sem',
            value: commentsPub7d,
            sig: commentsSig,
            metaText: 'meta 30d ≥ 20',
            detalhe: 'públicos · últimos 7d',
            help: 'Comentários públicos (visíveis ao cliente) na última semana. Mede uso conversacional vs uso de planilha.',
          },
          {
            key: 'triadas',
            label: 'Tasks triadas',
            value: pctTriadas + '%',
            sig: triadasSig,
            metaText: 'meta 30d ≥ 80%',
            detalhe: `${tasksTriadas} / ${tasksAtivas.length} ativas`,
            help: 'Tasks com responsável/cliente/prazo/esforço preenchidos conforme etapa. Mede disciplina de cadastro.',
          },
          {
            key: 'retention',
            label: 'Retention W1',
            ...
            sig: retentionSig,
            metaText: 'meta 30d ≥ 80%',
            detalhe: `${retidasCount} / ${ativasSemAnt.size} continuaram`,
            help: 'Pessoas ativas esta semana que TAMBÉM estavam ativas semana anterior. Métrica SaaS clássica de continuidade — indica hábito vs trial.',
          },
          {
            key: 'movimentacao',
            ...
            value: pctMov7d + '%',
            sig: movSig,
            metaText: 'meta 30d ≥ 60%',
            detalhe: movDeltaTxt,
            help: '% das tasks ativas (não-internas) que mudaram de subetapa/status nos últimos 7d. Mede "calor" do pipeline — se cair com DAU estável, há friction.',
          },
        ];

        // Conclusão heurística: avalia o conjunto. 6 indicadores agora.
        ...
        if (verdes === 6) {
          conclusao = 'Todos os 6 indicadores verdes. Adoção interna saudável — pronto pra avançar pra piloto externo controlado.';
          conclusaoSeveridade = 'verde';
        } else if (verdes >= 5 && vermelhos === 0) {
          const fraca = cards.find(c => c.sig === 'amarelo');
          conclusao = `${verdes} de 6 indicadores verdes. Foco em "${fraca.label}" pra fechar o gap — meta ${fraca.metaText.replace('meta 30d ', '')}.`;
          conclusaoSeveridade = 'amarelo';
        } else if (vermelhos === 0) {
          conclusao = `${verdes} de 6 verdes, ${amarelos} amarelo(s). Adoção crescendo mas ainda inconsistente — reforçar hábito diário antes de mais features.`;
          conclusaoSeveridade = 'amarelo';
        } else {
          const critica = cards.find(c => c.sig === 'vermelho');
          conclusao = `${vermelhos} indicador(es) crítico(s). Prioridade: "${critica.label}" (${critica.value}, ${critica.metaText.replace('meta 30d ', '')}). Investigar causa antes de mais features.`;
          conclusaoSeveridade = 'vermelho';
        }

        return { cards, conclusao, conclusaoSeveridade, verdes, amarelos, vermelhos };
      },
