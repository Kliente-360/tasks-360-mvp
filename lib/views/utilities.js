  ...
  function makeUtilitiesView() {
    return {
      ...
      handleGlobalShortcut(e) {
        ...
        // Cmd/Ctrl+Shift+N — captura rápida (funciona até digitando em campo).
        // Obs: o Chrome reserva Cmd+Shift+N (aba anônima) e pode não ceder o
        // atalho; a paleta (Cmd+K → "Captura rápida") é o caminho garantido.
        if ((e.metaKey || e.ctrlKey) && e.shiftKey && (e.key === 'n' || e.key === 'N')) {
          e.preventDefault();
          if (this.quickCaptureOpen) this.closeQuickCapture(); else this.openQuickCapture();
          return;
        }
      ...
      // ===================== CAPTURA RÁPIDA =====================
      // Overlay mínimo (só título) pra registrar uma tarefa em 2-5s sem
      // trocar de aba. A task entra em `backlog` sem cliente/responsável/
      // prazo — cai na Triagem pra alguém refinar depois.
      openQuickCapture() {
        this.quickCaptureOpen = true;
        this.quickCaptureTitle = '';
        this.track('quick_capture_open');
        this.$nextTick(() => {
          const el = document.getElementById('quickCaptureInput');
          if (el) el.focus();
        });
      },
      closeQuickCapture() {
      ...
      async submitQuickCapture() {
        const titulo = (this.quickCaptureTitle || '').trim();
        if (!titulo) return;
        this.quickCaptureTitle = '';
        ...
        const payload = {
          ...
          prazo: null, status: 'backlog', subetapa: 'backlog',
          tags: [], status_em: nowIso, subetapa_em: nowIso,
        };
        const { data, error } = await sb.from('tasks').insert(payload).select('*').single();
        if (error) { this.toast('error', 'Erro ao capturar: ' + error.message); return; }
        if (data) {
          ...
          this.track('quick_capture_create', {});
        }
        this.toast('success', 'Tarefa capturada — vai pra Triagem.');
        // Mantém aberto pra captura contínua (vários itens em sequência).
        this.$nextTick(() => {
          const el = document.getElementById('quickCaptureInput');
          if (el) el.focus();
        });
      ...
      get paletteResults() {
        ...
        const actions = [
          ...
          { id: 'act-quick',  kind: 'ação', label: 'Captura rápida',        hint: 'criar tarefa em 2s · ⌘⇧N',      action: () => this.openQuickCapture() },
