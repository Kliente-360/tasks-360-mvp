  ...
  function makeBriefingView() {
    return {
      ...
      get memoSumarioExecutivo() {
        ...
        if (cliRisco) {
          partes.push(`<strong>${escapeHtml(cliRisco.nome)}</strong> preocupa: ${escapeHtml(cliRisco.motivo)}. ${escapeHtml(cliRisco.acao)}.`);
        }
        // Frase 4: Top pessoa em sobrecarga
        const peRisco = this.briefingTimeRisco[0];
        if (peRisco) {
          partes.push(`<strong>${escapeHtml(peRisco.nome)}</strong> em ${peRisco.cargaNivel === 'sobrecarga' ? 'sobrecarga' : 'pressão'}${peRisco.pctCap != null ? ' (' + peRisco.pctCap + '%)' : ''} — ${escapeHtml(peRisco.acao)}.`);
        }
        // Frase 5: Sinal geral
      ...
      get memoSaudeClientes() {
        ...
        for (const c of this.reportClientHealth) {
          ...
          if (c.sinal === 'vermelho') {
            ...
            acao = '<strong>Conversar imediato.</strong> ' + (partes.length ? `Indicadores: ${partes.join(', ')}. ` : '') + escapeHtml(c.sinalReason || '');
          } else if (c.sinal === 'amarelo') {
            acao = `Atenção. ${escapeHtml(c.sinalReason || '')}` + (c.bloqAguard > 0 ? ` Reforçar follow-up.` : '');
          } else {
        ...
        for (const p of this._memoTeamLoadFull) {
          ...
          if (p.cargaNivel === 'sobrecarga') {
            ...
            if (sug && sug.candidate) {
              acao = `<strong>${p.pctCap}% capacidade</strong>, ${p.tasks} tasks${p.atrasadas > 0 ? ', ' + p.atrasadas + ' atrasadas' : ''}. <strong>Realocar</strong> "${escapeHtml(sug.task.titulo)}" para ${escapeHtml(sug.candidate.pessoa.nome)}.`;
            } else {
        ...
        if (teamWithCap.length >= 2) {
          ...
          if (variancia && variancia > 1.5) {
            ...
            out.push({
              ...
              detalhe: `Variância <strong>${variancia}x</strong> entre <strong>${escapeHtml(pMax.nome)}</strong> (${max}%) e <strong>${escapeHtml(pMin.nome)}</strong> (${min}%). Meta &lt; 1.5x. Sinal de gargalo individual.`,
              severidade: sev,
            });
          }
        }
          ...
          if (topId) {
            ...
            if (pct >= 50) {
              ...
              out.push({
                ...
                detalhe: `<strong>${pct}%</strong> das horas do time em <strong>${escapeHtml(nome)}</strong>. Risco de churn ↑ vs diversificação.`,
                severidade: sev,
              });
            }
          }
        ...
        if (subStats.length >= 2) {
          ...
          if (top.avg >= 4 && top.avg > second.avg * 1.5) {
            ...
            out.push({
              ...
              detalhe: `Tasks em <strong>"${escapeHtml(lbl)}"</strong> ficam <strong>${top.avg.toFixed(1)}d</strong> em média (${top.count} tasks). Próxima sub-etapa: ${second.avg.toFixed(1)}d. Possível falta de capacidade nessa fase.`,
              severidade: sev,
            });
          }
        }
      ...
      get memoDecisoes() {
        ...
        for (const p of projsEstourando) {
          out.push(`Renegociar escopo de <strong>${escapeHtml(p.nome)}</strong> (${escapeHtml(this.nomeCliente(p.clienteId))}) — ${p.pctEsgotamento}% executado.`);
        }
        ...
        for (const s of sustEstourando) {
          out.push(`Conversar contratual de <strong>${escapeHtml(s.nome)}</strong> (${escapeHtml(this.nomeCliente(s.clienteId))}) — sustentação estourando capacidade.`);
        }
        ...
        for (const p of sobrePessoas.slice(0, 2)) {
          const sug = this.weeklyRedistSuggestions.find(s => s.hit.pessoaId === p.id && s.reason !== 'ok');
          if (sug) {
            out.push(`Decidir realocação manual de <strong>${escapeHtml(p.nome)}</strong> — sem candidato automático (${escapeHtml(sug.reason)}).`);
          }
        }
        return out.slice(0, 6);
      },
      ...
      get memoSinaisPositivos() {
        ...
        if (cliVerdes.length >= 2) {
          const nomes = cliVerdes.slice(0, 3).map(c => escapeHtml(c.nome)).join(', ');
