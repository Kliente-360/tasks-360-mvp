  ...
  function makeCadastrosView() {
    return {
      ...
      async addCliente() {
        ...
        const { data, error } = await sb.from('clientes').insert({ nome: n }).select('id,nome,tier,arquivado_em,dominios,eh_interno,webhook_enabled').single();
        if (error) {
      ...
      openNewCliente() {
        ...
        // ehInterno é sempre false em cliente novo (flag só é setada via DB).
        this.editingCliente = { id: null, nome: '', tier: '', dominios: [], ehInterno: false };
        this.newDominio = '';
        this.modalCliente = true;
        this.$nextTick(() => this.$refs.clienteNomeInput && this.$refs.clienteNomeInput.focus());
      },
      openEditCliente(c) {
        this.editingCliente = {
          id: c.id,
          ...
          // ehInterno: clientes internos (bucket de gestão) não têm tier nem
          // dominios — UI esconde esses campos. Flag não é editável no modal.
          ehInterno: !!c.ehInterno,
        };
        this.newDominio = '';
        this.modalCliente = true;
      },
      // Domínios: normaliza pra lowercase, remove @ inicial e espaços.
      // Retorna '' se inválido (sem ponto = não é domínio). Validação básica
      // — o app não tenta resolver DNS, só formato superficial.
      _normalizeDominio(s) {
      ...
      addDominioCliente() {
        ...
        const d = this._normalizeDominio(this.newDominio);
        if (!d) { this.newDominio = ''; this.toast('error', 'Domínio inválido (ex: bodytech.com.br).'); return; }
        this.editingCliente.dominios = this.editingCliente.dominios || [];
        if (!this.editingCliente.dominios.includes(d)) {
          this.editingCliente.dominios.push(d);
        }
        this.newDominio = '';
        this.$nextTick(() => this.$refs.dominioInput && this.$refs.dominioInput.focus());
      },
      removeDominioCliente(d) {
        if (!this.editingCliente) return;
        this.editingCliente.dominios = (this.editingCliente.dominios || []).filter(x => x !== d);
      },
      async saveCliente() {
        ...
        // Dedupe + normalize na hora de salvar (defesa contra dirty state).
        // Clientes internos (bucket de gestão) nunca persistem tier/dominios —
        // mesmo se o state local tiver algo legado, força null/[] no payload.
        const isInterno = !!e.ehInterno;
        const dominios = isInterno ? [] : Array.from(new Set((e.dominios || []).map(d => this._normalizeDominio(d)).filter(Boolean)));
        const tier = isInterno ? null : (e.tier || null);
        const payload = { nome, tier, dominios };

        // Modo CRIAR (sem id) — insert otimista
        if (!e.id) {
          const tempId = 'tmp-' + Math.random().toString(36).slice(2, 8);
          const optimistic = { id: tempId, nome, tier: tier || '', dominios: [...dominios], arquivadoEm: null };
          this.clientes = [...this.clientes, optimistic].sort((a,b) => a.nome.localeCompare(b.nome));
          this.modalCliente = false;
          const { data, error } = await sb.from('clientes').insert(payload).select('id,nome,tier,arquivado_em,dominios,eh_interno,webhook_enabled').single();
          if (error) {
            this.clientes = this.clientes.filter(c => c.id !== tempId);
        ...
        if (i >= 0) this.clientes[i] = { ...prev, nome, tier: tier || '', dominios: [...dominios] };
        this._dataRev++;
        this.modalCliente = false;
      ...
      async arquivarTask(t) {
        ...
        const prev = this._patchTask(id, { arquivadoEm: nowIso });
        if (!prev) return;
        ...
        if (error) { this._replaceTask(id, prev); this.toast('error', 'Erro: ' + error.message); return; }
        this.toast('success', `"${prev.titulo}" arquivada.`);
      },
      async desarquivarTask(t) {
        const id = (t && t.id) || (this.editing && this.editing.id);
        if (!id) return;
        const prev = this._patchTask(id, { arquivadoEm: null });
        if (!prev) return;
      ...
      async bulkArquivar() {
        ...
        this._patchTasks(ids, { arquivadoEm: nowIso });
        this.selectedIds = [];
      ...
      async _doArquivarCliente(c, projsAtivos, tasksAtivas) {
        ...
        if (tasksAtivas.length) {
          this._patchTasks(tasksAtivas.map(t => t.id), { arquivadoEm: nowIso });
        }
        this._dataRev++;
      ...
      async deleteCliente(c) {
        if (c.ehInterno) {
          this.toast('error', 'Cliente interno não pode ser excluído (bucket de gestão).');
          return;
        }
      ...
      openNewProjeto() {
        ...
        this.$nextTick(() => this.$refs.projetoNomeInput && this.$refs.projetoNomeInput.focus());
      },
      openEditProjeto(p) {
        this.editingProjeto = {
          id: p.id,
      ...
      openNewPessoa() {
        ...
        this.$nextTick(() => this.$refs.pessoaNomeInput && this.$refs.pessoaNomeInput.focus());
      },
