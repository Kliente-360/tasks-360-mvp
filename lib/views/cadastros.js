  ...
  function makeCadastrosView() {
    return {
      ...
      openNewCliente() {
        ...
        this.editingCliente = { id: null, nome: '', tier: '', dominios: [] };
        this.newDominio = '';
      ...
      async saveCliente() {
        ...
        const dominios = Array.from(new Set((e.dominios || []).map(d => this._normalizeDominio(d)).filter(Boolean)));
        const payload = { nome, tier: e.tier || null, dominios };

        // Modo CRIAR (sem id) — insert otimista
        if (!e.id) {
          const tempId = 'tmp-' + Math.random().toString(36).slice(2, 8);
          const optimistic = { id: tempId, nome, tier: e.tier || '', dominios: [...dominios], arquivadoEm: null };
          this.clientes = [...this.clientes, optimistic].sort((a,b) => a.nome.localeCompare(b.nome));
        ...
        if (i >= 0) this.clientes[i] = { ...prev, nome, tier: e.tier || '', dominios: [...dominios] };
        this._dataRev++;
        this.modalCliente = false;
