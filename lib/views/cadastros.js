  ...
  function makeCadastrosView() {
    return {
      ...
      async addCliente() {
        ...
        const { data, error } = await sb.from('clientes').insert({ nome: n }).select('id,nome,tier,arquivado_em,dominios,eh_interno').single();
        if (error) {
      ...
      async saveCliente() {
        ...
        if (!e.id) {
          ...
          const { data, error } = await sb.from('clientes').insert(payload).select('id,nome,tier,arquivado_em,dominios,eh_interno').single();
          if (error) {
            this.clientes = this.clientes.filter(c => c.id !== tempId);
